...
uri = "mongodb+srv://ASSAN:A.emr!12@batterymanagementcluste.wyc4v.mongodb.net/?retryWrites=true&w=majority&appName=BatteryManagementCluster"
        ...
        if indexTrack > 6+ 2:
            ...
            if sum % 9 == byte_int:
                ...
                insert_to_db("BatteryManagement", "logs",
                             {"data": {tuple(resultList)}, "Code": 400, "time": time.strftime("%d.%m.%Y")})
                sum = 0
                resultList = []
        else:
    ...
    while True:
        try:
            ...
            if(int(data[2], 16) == 10):
                ...
                print()
                print("Burada B degerini okumasÄ± lazim")
                print()
...
def monitor_collection(mongo_uri, database_name, collection_name):
    # MongoDB'ye baglanti
    ...
    print("Change stream basladi.")

    try:
        while True:
            print("Degisiklik bekleniyor...")
            change = change_stream.next()
            print(f"Degisiklik algilandi: {change}")
            if change['operationType'] == 'insert':
                print("Yeni veri eklendi:")
                print(f"slaveAddress: {change['fullDocument']['slaveAddress']}")
                print(f"slaveCommand: {change['fullDocument']['slaveCommand']}")
                address = change['fullDocument']['slaveAddress']
                command = change['fullDocument']['slaveCommand']
...
def main():
    ...
    #gonderilecek_veri = monitor_collection(uri, "BatteryManagement", "datagets")
    #threading.Thread(target=send_data_uart,args=('/dev/serial0',115200,gonderilecek_veri),daemon=True).start()
    threading.Thread(target= check_input,daemon=True).start()
    #monitor_collection(uri, "BatteryManagement", "datagets")
    #threading.Thread(target=monitor_collection, args=(uri, "BatteryManagement", "datagets"), daemon=True).start()
